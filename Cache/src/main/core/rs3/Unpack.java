...
import core.rs3.js4.Jagfile;
import core.rs3.js5.*;
import core.rs3.unpack.*;
import core.rs3.unpack.config.*;
import core.rs3.unpack.config.TextureUnpacker;
import core.rs3.unpack.cutscene2d.Cutscene2D;
import core.rs3.unpack.font.FontMetrics;
import core.rs3.unpack.map.MapSquare;
import core.rs3.unpack.script.Command;
import core.rs3.unpack.script.ScriptUnpacker;
import core.rs3.unpack.uianim.Anim;
import core.rs3.unpack.uianim.AnimCurve;
import core.rs3.unpack.unknown62.AnimatorController;
import core.rs3.unpack.vfx.VFXUnpacker;
import core.rs3.unpack.worldmap.WorldMapUnpacker;
...
//        unpackRS2(530,254);
//        unpackLocal("resources/unpacked", 530, -1);

    }

    /**
     * Unpacks a local flat file cache using FlatFileResourceProvider.
     *
     * @param path    Path to files folder
     * @param version Cache revision/version
     * @param id      Optional scope
     * @throws IOException
     */
    public static void unpackLocal(String path, int version, int id) throws IOException {
        VERSION = version;
        ID = id;

        unpack(Path.of(path), new MemoryCacheResourceProvider(new FileSystemCacheResourceProvider(
                Path.of("resources/cache"),
                new FlatFileResourceProvider(Path.of("resources/flat_files")))
        ));
    }
    ...
    public static void unpackLive(String path, int version, int subversion, int language, String host, int port, String token) throws IOException {
        ...
        unpack(Path.of(path), new MemoryCacheResourceProvider(new FileSystemCacheResourceProvider(
                Path.of(System.getProperty("user.home") + "/.rscache/core.rs3"),
