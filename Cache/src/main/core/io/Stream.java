...
public class Stream {
    ...
    final boolean compareCrcs() {
        offset -= 4;
        CRC32Instance.reset();
        CRC32Instance.update(payload, 0, offset);
        int crc = (int) CRC32Instance.getValue();
        int realCRC = getInt();
        return realCRC == crc;
    }
    ...
    final String getJStr() {
        ...
        if (k == 0) {
            ...
            return ByteBufferUtils.getStringFromBytes(payload, j, k);
        }
    }
    ...
    final int putCrc(int off) {
        CRC32Instance.reset();
        CRC32Instance.update(payload, off, offset);
        int crc = (int) CRC32Instance.getValue();
        putInt(crc);
        return crc;
    }
    ...
    final void putJStr(String s) {
        ...
        offset += ByteBufferUtils.getStringBytes(s, 0, s.length(), payload, offset);
        payload[offset++] = 0;
    }
