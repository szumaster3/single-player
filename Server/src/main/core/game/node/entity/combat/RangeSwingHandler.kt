...
open class RangeSwingHandler(vararg flags: SwingHandlerFlag) : CombatSwingHandler(CombatStyle.RANGE, *flags) {
    ...
    override fun swing(entity: Entity?, victim: Entity?, state: BattleState?): Int {
        ...
        var hit = 0
        val armourPierce =
            state.ammunition != null && state.ammunition.effect != null && state.ammunition.effect == BoltEffect.DIAMOND && state.ammunition.effect!!.canFire(
                state
            )
    ...
    fun configureRangeData(entity: Entity?, state: BattleState?) {
        ...
        if (entity is Player) {
            ...
            state.ammunition = w.ammunition?.let { Ammunition.get(it.id) }
        } else {
            w = Weapon(null)
        }
        state.weapon = w
    ...
    override fun visualize(entity: Entity, victim: Entity?, state: BattleState?) {
        ...
        if (state!!.ammunition != null) {
            ...
            if (state.weapon.type == WeaponType.DOUBLE_SHOT && state.ammunition.darkBowGraphics != null) {
                ...
                state.ammunition.projectile?.let {
                    Projectile.create(
                        entity,
                        victim,
                        it.projectileId,
    ...
    override fun impact(entity: Entity?, victim: Entity?, state: BattleState?) {
        if (state!!.ammunition != null && state.ammunition.effect != null && state.ammunition.effect != BoltEffect.DIAMOND && state.ammunition.effect!!.canFire(
                state
            )
        ) {
            state.ammunition.effect!!.impact(state)
        ...
        when (entity) {
            is Player -> {
                if (entity.equipment[EquipmentContainer.SLOT_WEAPON] != null && RangeWeapon.get(entity.equipment[EquipmentContainer.SLOT_WEAPON].id)!!.ammunitionSlot != EquipmentContainer.SLOT_ARROWS && entity.equipment[EquipmentContainer.SLOT_ARROWS] != null)
                    styleStrengthBonus -= entity.equipment[EquipmentContainer.SLOT_ARROWS].definition.getConfiguration<IntArray>(
                        ItemConfigParser.BONUS
                    )[14]
                ...
                if (!flags.contains(SwingHandlerFlag.IGNORE_PRAYER_BOOSTS_DAMAGE))
                    effectiveStrengthLevel =
                        floor(effectiveStrengthLevel + (entity.prayer.getSkillBonus(Skills.RANGE) * effectiveStrengthLevel))
    ...
    override fun calculateDefence(victim: Entity?, attacker: Entity?): Int {
        ...
        when (victim) {
            is Player -> {
                ...
                effectiveDefLevel =
                    floor(effectiveDefLevel + (victim.prayer.getSkillBonus(Skills.DEFENCE) * effectiveDefLevel))
