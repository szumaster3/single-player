...
public class IfaceDefinition {
    ...
    public static byte[] encodeIf1(IfaceDefinition def) {
        IoBuffer buf = new IoBuffer();

        buf.p1(0);
        buf.p1(def.type.ordinal());
        buf.p1(def.buttonType);
        buf.p2(def.clientCode);
        buf.p2(def.baseX);
        buf.p2(def.baseY);
        buf.p2(def.baseWidth);
        buf.p2(def.baseHeight);
        buf.p1(def.alpha);
        buf.p2(def.overlayer == -1 ? 65535 : (def.overlayer & 0xFFFF));
        buf.p2(def.unknownProp_11);

        if (def.cs1ComparisonOpcodes != null) {
            buf.p1(def.cs1ComparisonOpcodes.length);
            for (int i = 0; i < def.cs1ComparisonOpcodes.length; i++) {
                buf.p1(def.cs1ComparisonOpcodes[i]);
                buf.p2(def.cs1ComparisonOperands[i]);
            }
        } else buf.p1(0);

        if (def.cs1Scripts != null) {
            buf.p1(def.cs1Scripts.length);
            for (int[] script : def.cs1Scripts) {
        ...
        switch (def.type) {
            case SCROLLABLE:
                buf.p2(def.scrollMaxV);
                buf.p1(def.hidden ? 1 : 0);
                break;
            ...
            case UNKNOWN_2:
                buf.p1(def.baseWidth);
                buf.p1(def.baseHeight);
                buf.p1(def.baseWidth);
                buf.p1(def.baseHeight);
                buf.p1(0);
                buf.p1(0);
                buf.p1(0);
                buf.p1(0);
                buf.p1(def.invMarginX);
                buf.p1(def.invMarginY);

                for (int i = 0; i < 20; i++) {
                    if (def.invSprite != null && def.invSprite[i] != -1) {
                        buf.p1(1);
                        buf.p2(def.invOffsetX[i]);
                        buf.p2(def.invOffsetY[i]);
                        buf.p4(def.invSprite[i]);
                    } else {
                        buf.p1(0);
                    }
                }
                for (int i = 0; i < 5; i++) {
                    buf.putJagString(def.invOptions != null && def.invOptions[i] != null ? def.invOptions[i] : "");
                }
                break;

            case UNKNOWN_3:
                buf.p1(def.filled ? 1 : 0);
                buf.p4(def.color);
                buf.p4(def.activeColor);
                buf.p4(def.overColor);
                buf.p4(def.unknownColor);
                break;
            ...
            case TEXT:
                ...
                buf.p1(def.valign);
                buf.p1(def.vPadding);
                buf.p2(def.font == -1 ? 65535 : def.font);
                buf.p1(def.shadowed ? 1 : 0);
                buf.putJagString(def.text == null ? "" : def.text);
                buf.putJagString(def.activeText == null ? "" : def.activeText);
                buf.p4(def.color);
                buf.p4(def.activeColor);
                buf.p4(def.overColor);
                buf.p4(def.unknownColor);
            ...
            case SPRITE:
                buf.p4(def.spriteId);
                buf.p4(def.activeSpriteId);
                break;

            case MODEL:
                buf.p2(def.modelId == -1 ? 65535 : def.modelId);
                buf.p2(def.activeModelId == -1 ? 65535 : def.activeModelId);
                buf.p2(def.modelAnimId == -1 ? 65535 : def.modelAnimId);
                buf.p2(def.activeModelAnimId == -1 ? 65535 : def.activeModelAnimId);
                buf.p2(def.modelZoom);
                buf.p2(def.modelXAngle);
                buf.p2(def.modelYAngle);
                break;
            ...
            case UNKNOWN_7:
                ...
                buf.p1(def.shadowed ? 1 : 0);
                buf.p4(def.color);
                buf.p2(def.invMarginX);
                buf.p2(def.invMarginY);
                buf.p1(def.invOptions != null ? 1 : 0);
                for (int i = 0; i < 5; i++)
                    buf.putJagString(def.invOptions != null && def.invOptions[i] != null ? def.invOptions[i] : "");
                break;
        ...
        if (def.buttonType == 2 || def.type == ComponentType.UNKNOWN_2) {
            buf.putJagString(def.opCircumfix == null ? "" : def.opCircumfix);
            buf.putJagString(def.opSuffix == null ? "" : def.opSuffix);
            buf.p2(def.unknownProp_9 & 0x3F);
        }

        if (def.buttonType == 1 || def.buttonType == 4 || def.buttonType == 5 || def.buttonType == 6) {
            buf.putJagString(def.option == null ? "" : def.option);
        }

        return buf.array();
    }

    private static void encodeIf3Type(IfaceDefinition def, IoBuffer buf) {
        switch (def.type) {
            case SCROLLABLE:
                buf.p2(def.scrollMaxH);
                buf.p2(def.scrollMaxV);
                buf.p1(def.noClickThrough ? 1 : 0);
                break;
            case SPRITE:
                buf.p4(def.spriteId);
                buf.p2(def.angle2d);
                int spriteFlags = 0;
                if (def.spriteTiling) spriteFlags |= 1;
                if (def.hasAlpha) spriteFlags |= 2;
                buf.p1(spriteFlags);
                buf.p1(def.alpha);
                buf.p1(def.outlineThickness);
                buf.p4(def.shadowColor);
                buf.p1(def.vFlip ? 1 : 0);
                buf.p1(def.hFlip ? 1 : 0);
                break;
            case MODEL:
                buf.p2(def.modelId == -1 ? 65535 : def.modelId);
                buf.p2(def.unknownModelProp_1);
                buf.p2(def.unknownModelProp_2);
                buf.p2(def.modelXAngle);
                buf.p2(def.modelYAngle);
                buf.p2(def.modelYOffset);
                buf.p2(def.modelZoom);
                buf.p2(def.modelAnimId == -1 ? 65535 : def.modelAnimId);
                buf.p1(def.modelOrtho ? 1 : 0);
                buf.p2(def.unknownModelProp_3);
                buf.p2(def.unknownModelProp_4);
                buf.p1(def.unknownModelProp_5 ? 1 : 0);
                if (def.dynWidth != 0) buf.p2(def.unknownModelProp_6);
                if (def.dynHeight != 0) buf.p2(def.unknownModelProp_7);
                break;
            case TEXT:
                buf.p2(def.font == -1 ? 65535 : def.font);
                buf.putJagString(def.text == null ? "" : def.text);
            ...
            case UNKNOWN_3:
                ...
                break;
            case UNKNOWN_9:
                buf.p1(def.lineWidth);
                buf.p4(def.color);
                buf.p1(def.unknownProp_8 ? 1 : 0);
                break;
        }
    }

    private static void encodeIf3ScriptArgs(ScriptArgs args, IoBuffer buf) {
        if (args == null) {
            buf.p1(0);
            return;
        }

        Object[] all = new Object[args.args.length + 1];
        all[0] = args.id;
        System.arraycopy(args.args, 0, all, 1, args.args.length);
        ...
        for (Object o : all) {
            if (o instanceof Integer) {
                ...
                buf.p1(1);
                buf.putJagString((String) o);
            }
        }
    }

    public static byte[] encodeIf3(IfaceDefinition def) {
        IoBuffer buf = new IoBuffer();
        ...
        buf.p2(def.baseHeight);
        buf.p1(def.dynWidth);
        buf.p1(def.dynHeight);
        buf.p1(def.yMode);
        buf.p1(def.xMode);
        buf.p2(def.overlayer == -1 ? 65535 : (def.overlayer & 0xFFFF));
        buf.p1(def.hidden ? 1 : 0);

        encodeIf3Type(def, buf);
        // unknownLocal_1
        buf.p3(0);
        // unknownLocal_2
        buf.p1(0);

        buf.putJagString(def.optionBase == null ? "" : def.optionBase);

        int opCount = def.ops != null ? def.ops.length : 0;
        buf.p1(opCount);
        for (int i = 0; i < opCount; i++)
            buf.putJagString(def.ops[i]);

        buf.p1(def.dragDeadzone);
        buf.p1(def.dragDeadtime);
        buf.p1(def.dragRenderBehavior ? 1 : 0);
        buf.putJagString(def.opCircumfix == null ? "" : def.opCircumfix);

        buf.p2(def.unknownProp_9 == -1 ? 65535 : def.unknownProp_9);
        buf.p2(def.unknownProp_10 == -1 ? 65535 : def.unknownProp_10);

        encodeIf3ScriptArgs(def.scripts.unknown, buf);
        encodeIf3ScriptArgs(def.scripts.onMouseOver, buf);
        encodeIf3ScriptArgs(def.scripts.onMouseLeave, buf);
        encodeIf3ScriptArgs(def.scripts.onUseWith, buf);
        encodeIf3ScriptArgs(def.scripts.onUse, buf);
        encodeIf3ScriptArgs(def.scripts.onVarpTransmit, buf);
        encodeIf3ScriptArgs(def.scripts.onInvTransmit, buf);
        encodeIf3ScriptArgs(def.scripts.onStatTransmit, buf);
        encodeIf3ScriptArgs(def.scripts.onTimer, buf);
        encodeIf3ScriptArgs(def.scripts.onOptionClick, buf);
        encodeIf3ScriptArgs(def.scripts.onMouseRepeat, buf);
        encodeIf3ScriptArgs(def.scripts.onClickRepeat, buf);
        encodeIf3ScriptArgs(def.scripts.onDrag, buf);
        encodeIf3ScriptArgs(def.scripts.onRelease, buf);
        encodeIf3ScriptArgs(def.scripts.onHold, buf);
        encodeIf3ScriptArgs(def.scripts.onDragStart, buf);
        encodeIf3ScriptArgs(def.scripts.onDragRelease, buf);
        encodeIf3ScriptArgs(def.scripts.onScroll, buf);
        encodeIf3ScriptArgs(def.scripts.onVarcTransmit, buf);
        encodeIf3ScriptArgs(def.scripts.onVarcstrTransmit, buf);

        encodeIf3Triggers(def.triggers.varpTriggers, buf);
        encodeIf3Triggers(def.triggers.inventoryTriggers, buf);
        encodeIf3Triggers(def.triggers.statTriggers, buf);
        encodeIf3Triggers(def.triggers.varcTriggers, buf);
        encodeIf3Triggers(def.triggers.varcstrTriggers, buf);

        return buf.array();
    }

    private static void encodeIf3Triggers(int[] triggers, IoBuffer buf) {
        if (triggers == null) {
            buf.p1(0);
            return;
        }
        buf.p1(triggers.length);
        for (int t : triggers)
            buf.p4(t);
    }
