...
class DesertTreasure : Quest(Quests.DESERT_TREASURE, 45, 44, 3, 440, 358, 0, 1, 15) {
    ...
    companion object {
        ...
        const val varbitDesertTreasure =
            358 // 10-15-Eblis 1924 shows up other values Eblis disappears. This is tied to the quest.
        ...
        const val varbitRingOfVisibility =
            393 // Set to 1 to show ladder and other cool stuff when wearing ring of visibility.
        ...
        fun completedAllSubstages(player: Player): Boolean {
            return getSubStage(player, attributeBloodStage) == 100 && getSubStage(
                player,
                attributeSmokeStage
            ) == 100 && getSubStage(player, attributeIceStage) == 100 && getSubStage(
                player,
                attributeShadowStage
            ) == 100
        }
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (!started) {
            ...
            line(
                player, "I must have completed the following quests:", line++
            ...
            if (stage >= 9 && completedAllSubstages(player)) {
                ...
                if (getSubStage(player, attributeBloodStage) == 100) {
                    ...
                    if (getSubStage(player, attributeBloodStage) >= 3) {
                        ...
                        line(
                            player, "I then need to take that !!sacrificial pot?? to !!Entrana?? and get", line++, false
                        )
                        ...
                        line(
                            player, "I then need to add !!garlic?? and !!spices?? to the pot in order to", line++, false
                        )
                ...
                if (getSubStage(player, attributeShadowStage) == 100) {
                    ...
                    if (getSubStage(player, attributeShadowStage) >= 3) {
                        ...
                        line(
                            player, "I need to find !!Laheeb's loot?? and retrieve the stolen !!gilded??", line++, false
                        )
    ...
    override fun reset(player: Player) {
        ...
        setVarbit(
            player, varbitChildReunite, 0, true
