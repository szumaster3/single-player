...
class JaldraochtPyramid: MapArea {
    ...
    override fun entityStep(entity: Entity, location: Location, lastLocation: Location) {
        ...
        for (i in baseFloors.indices) {
            if (player.location.isInRegion(i)){
                ...
                if ((1..20).random() == 1) {
                    val sarcoph = nearSarcophagus(entity.location)
                    if (sarcoph != null) {
                        spawnMummy(entity, sarcoph)
                    }
                }
        ...
        if(player.location.isInRegion(12945)){
            if (!safeZone.insideBorder(entity.location)) {
                if ((1..20).random() == 1) {
                    val sarcoph = nearSarcophagus(entity.location)
                    if (sarcoph != null) {
                        spawnMummy(entity, sarcoph)
    ...
    companion object {
        ...
        @JvmStatic
        private fun getNewLocation(direction: Direction): Location =
            when (direction) {
        ...
        @JvmStatic
        private fun getNewFacing(direction: Direction): Direction =
            when (direction) {
        ...
        @JvmStatic
        fun spawnMummy(player: Player, loc: Location) {
            val sarcophagusScenery = getScenery(loc) ?: return
