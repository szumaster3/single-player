...
class AliTheSmithDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            43 -> npcl(FaceAnim.LAUGH, "They're just worried that they'll lose money when we open our shops.").also { stage = 44 }
            44 -> end()
        }
        return true
    }

    override fun newInstance(player: Player?): Dialogue = AliTheSmithDialogue(player)

    override fun getIds(): IntArray = intArrayOf(NPCs.ALI_THE_SMITH_2820)
}
