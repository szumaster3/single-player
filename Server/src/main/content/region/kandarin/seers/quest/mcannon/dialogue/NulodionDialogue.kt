    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            ...
            80 ->
                when (stage) {
                    0 -> {
                        if (!player.hasItem(content.region.kandarin.seers.quest.mcannon.DwarfCannon.NULODION_NOTES)) {
                            player("I've lost the notes.")
                            stage = 100
                        }
                        if (!player.hasItem(content.region.kandarin.seers.quest.mcannon.DwarfCannon.MOULD)) {
