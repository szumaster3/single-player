...
class GuardDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        val KILLER = when (getAttribute(player!!, MurderMystery.attributeRandomMurderer, 0)) {
            0 -> "Anna"
            1 -> "Bob"
            2 -> "Carol"
            3 -> "David"
        ...
        val POISONSPOT = when (getAttribute(player!!, MurderMystery.attributeRandomMurderer, 0)) {
            0 -> "compost heap"
            1 -> "beehive"
            2 -> "drain"
            3 -> "spiders nest"
        ...
        val KILLERMALE = when (getAttribute(player!!, MurderMystery.attributeRandomMurderer, 0)) {
            1, 3, 5 -> true
            else -> false
        }
        val EVIDENCE = (1796..1822).toList().toIntArray()

        when (getQuestStage(player!!, Quests.MURDER_MYSTERY)) {
            0 -> when (stage) {
                0 -> playerl(FaceAnim.NEUTRAL, "What's going on here?").also { stage++ }
                1 -> npcl(
                    FaceAnim.SAD,
                    "Oh, it's terrible! Lord Sinclair has been murdered and we don't have any clues as to who or why. We're totally baffled!"
                ).also { stage++ }

                2 -> npcl(FaceAnim.SAD, "If you can help us we will be very grateful.").also { stage++ }
                3 -> options("Sure, I'll help.", "You should do your own dirty work.").also { stage++ }
                4 -> when (buttonID) {
                    1 -> playerl(FaceAnim.HAPPY, "Sure, I'll help.").also { stage++ }
                    2 -> playerl("You should do your own dirty work.").also { stage = 8 }
                }

                5 -> npcl(FaceAnim.HAPPY, "Thanks a lot!").also { stage++ }
                6 -> playerl(FaceAnim.NEUTRAL, "What should I be doing to help?").also { stage++ }
                7 -> npcl(
                    FaceAnim.NEUTRAL,
                    ...
                    .also { setAttribute(player!!, MurderMystery.attributeRandomMurderer, RandomFunction.random(5)) }
                    .also { setQuestStage(player!!, Quests.MURDER_MYSTERY, 1) }
                    .also { stage = END_DIALOGUE }

                8 -> npcl("Get lost then, this is private property! ...Unless you'd like to be taken in for questioning yourself?").also {
                    stage = END_DIALOGUE
                }
            }
            ...
            1 -> when (stage) {
                ...
                1 -> when (buttonID) {
                    ...
                    3 -> playerl(FaceAnim.HAPPY, "I know who did it!")
                        .also {
                            stage = if (MurderMystery.solvedMystery(player!!)) 46
                            else if (MurderMystery.clueCount(player!!) == 1) {
                                if (inInventory(player!!, Items.KILLERS_PRINT_1815)) {
                                    41
                                } else if (getAttribute(player!!, MurderMystery.attributePoisonClue, 0) == 2) {
                                    37
                                } else 33
                            } else if (MurderMystery.clueCount(player!!) > 1) 32
                            else 6
                        }
                }

                2 -> npcl("Look around and investigate who might be responsible. The Sarge said every murder leaves clues to who done it, but frankly we're out of our depth here.").also {
                    stage = END_DIALOGUE
                }
                ...
                4 -> npcl("The door was found locked from the inside, and he seemed to have been stabbed, but there was an odd smell in the room. Frankly, I'm stumped.").also {
                    stage = END_DIALOGUE
                }
                ...
                9 -> npcl("Although having said that the butler is kind of shifty looking...").also {
                    stage = END_DIALOGUE
                }
                ...
                12 -> npcl("and the case is closed, but the murdered man was a friend of the King and it's more than my job's worth not to investigate fully.").also {
                    stage = END_DIALOGUE
                }
                ...
                32 -> showTopics(
                    ...
                    IfTopic("I have proof one of the family lied about the poison.", 37, getAttribute(player!!,
                        MurderMystery.attributePoisonClue, 0) == 2, true),
                ...
                46 -> playerl(FaceAnim.HAPPY, "I have conclusive proof who the killer was.").also { stage++ }
                47 -> npcl(FaceAnim.HAPPY, "You do? That's excellent work. Let's hear it then.").also { stage++ }
                ...
                50 -> playerl(FaceAnim.HAPPY, "Nobody heard the guard dog barking, which it would have if it had been an intruder who was responsible.").also { stage++ }
                ...
                58 -> npcl(
                    FaceAnim.HAPPY,
                    "Yes. There's no doubt about it. It must have been $KILLER who killed " + if (KILLERMALE) {
                        "his"
                    } else {
                        "her"
                    } + " father. All of the guards must congratulate you on your excellent work in helping us to solve this case."
                ).also { stage++ }

                59 -> npcl(
                    "We don't have many murders here in ${GameWorld.settings?.name} and I'm afraid we wouldn't have been able to solve it by ourselves. We will hold " + if (KILLERMALE) {
                        "him"
                    } else {
                        "her"
                    } + " here under house arrest until such time as we bring " + if (KILLERMALE) {
                        "him"
                    } else {
                        "her"
                    } + " to trial."
                ...
                62 -> npcl(FaceAnim.HAPPY, "Please accept this reward from the family!").also { stage++ }
