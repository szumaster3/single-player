    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, Quests.TOWER_OF_LIFE)) {
            0 -> when (stage) {
                ...
                2  -> npcl(FaceAnim.FRIENDLY, "You like it? It's simply fabulous, isn't it? A real marvel of modern design made just for my fellow alchemists and I!").also { stage++ }
                3  -> playerl(FaceAnim.FRIENDLY, "I see some builders over there. So I assume it's not yet finished.").also { stage++ }
                4  -> npcl(FaceAnim.FRIENDLY, "What a keen eye this " + (if (player.isMale) "LAD" else "LASS") + " has! But imagine this: a gargantuan cylinder of expert design, incorporating an inventive inner spiral walkway to multiple planes of wondrous inhabitance!").also { stage++ }
                ...
                12 -> npcl(FaceAnim.FRIENDLY, "They've gone on strike for some petty reason about the tower being too weird. I really can't understand those men.").also { stage++ }
                13 -> playerl(FaceAnim.FRIENDLY, "Shame.").also { stage++ }
                14 -> npcl(FaceAnim.FRIENDLY, "Hey! Maybe you could talk to them?").also { stage++ }
                15 -> npcl(FaceAnim.FRIENDLY, "You look like a sturdy " + (if (player.isMale) "LAD" else "LASS") + ".").also { stage++ }
                ...
                20 -> when (buttonID) {
                    1 -> playerl(FaceAnim.FRIENDLY, "Sure, why not.").also { stage++ }
                ...
                25 -> npcl(FaceAnim.FRIENDLY, "Go have a word with Bonafido the head builder. See if you can't get him off his backside to do some work. Oh, and I'm Effigy by the way.").also { stage++ }
                26 -> playerl(FaceAnim.FRIENDLY, "Nice to meet you Effigy, I'm ${player.username}. Hold on and I'll see what I can do!").also { stage++ }
                27 -> {
                    end()
                    setQuestStage(player, Quests.TOWER_OF_LIFE, 1)
                    setVarbit(player, Vars.VARBIT_TOWER_OF_LIFE_PROGRESS_3337, 1, true)
                }
            }
            ...
            3 -> if (getAttribute(player, GameAttributes.TOL_TOWER_ACCESS, 0) == 1) {
                when (stage) {
                    ...
                    4 -> playerl(FaceAnim.FRIENDLY, "I'm not sure about that, but I'll take a look and see what I can do.").also { stage++ }
            ...
            6 -> if (getAttribute(player, GameAttributes.TOL_CUTSCENE, false)) {
                when (stage) {
                    ...
                    1  -> npcl(FaceAnim.FRIENDLY, "I know, I know. This is a truly dangerous creation, of all proportions.").also { stage++ }
                    2  -> playerl(FaceAnim.FRIENDLY, "I hope you learn from this.").also { stage++ }
                    3  -> npcl(FaceAnim.FRIENDLY, "I will, I will. Next time we'll use a stronger cage.").also { stage++ }
                    ...
                    7  -> playerl(FaceAnim.FRIENDLY, "So, what about that Homunculus up there? You can't just leave it.").also { stage++ }
                    ...
                    14 -> npcl(FaceAnim.FRIENDLY, "I'll make sure you're compensated. You won't be forgotten for this!").also { stage++ }
                    15 -> playerl(FaceAnim.FRIENDLY, "Why do I get the feeling I'm doomed to go face that creature on my own?").also { stage++ }
                    16 -> npcl(FaceAnim.FRIENDLY, "Good! I knew you would help us!").also { stage++ }
                    17 -> playerl(FaceAnim.FRIENDLY, "Oh well. My mum did always tell me that experience teaches fools.").also { stage = END_DIALOGUE }
                }
            }
            ...
            8 -> when (stage) {
                ...
                8  -> npcl(FaceAnim.FRIENDLY, "Me not hurt. Now tell. What you plans? What you do with me?").also { stage++ }
                9  -> npcl(FaceAnim.FRIENDLY, "Please, we just wanted to experiment. We wanted to create life. There's a dungeon under the tower. Long ago, we found a strange source of energy that we believe was left behind by the great Guthix. He used to").also { stage++ }
                10 -> npcl(FaceAnim.FRIENDLY, "create life with such ease. We realised we should be able to use this powerful substance, but we needed to bring together our magic along with the logical construction of the builders. It seemed through our experiments that").also { stage++ }
                11 -> npcl(FaceAnim.FRIENDLY, "even magic needs a certain level of rules and laws to work.").also { stage++ }
                ...
                14 -> npcl(FaceAnim.FRIENDLY, "Go look dungeon, ${player.username}. Please meet there.").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
