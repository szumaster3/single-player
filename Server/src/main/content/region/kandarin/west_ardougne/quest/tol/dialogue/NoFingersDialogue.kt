...
class NoFingersDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int, ): Boolean {
        when (getQuestStage(player!!, "Tower of Life")) {
            in 0..1 -> when (stage) {
                START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "You seem to like playing with danger, there.").also { stage++ }
                1 -> npcl(FaceAnim.FRIENDLY, "They don't call me 'No-fingers' for nothing!").also { stage++ }
                2 -> playerl(FaceAnim.FRIENDLY, "Well, correct me if I'm wrong, but you seem to have all your fingers.").also { stage++ }
                3 -> npcl(FaceAnim.FRIENDLY, "It may seem that way mate, but I think you'll find I don't have any.").also { stage++ }
                ...
                10 -> playerl(FaceAnim.FRIENDLY, "Yes, well I think I'd better be going now.").also { stage = END_DIALOGUE }
            }

            2 -> when (stage) {
                START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "Hello. I was just wondering... I don't suppose you have any spare clothing you could lend me?").also { stage++ }
                ...
                5 -> npcl(FaceAnim.FRIENDLY, "Nope. Only real builders can wear builders' boots, and you're not even close.").also { stage = END_DIALOGUE }
            }

            3 -> if (getAttribute(player, GameAttributes.TOL_TOWER_ACCESS, 0) == 1) {
                when (stage) {
                    START_DIALOGUE -> playerl(FaceAnim.FRIENDLY, "Hi.").also { stage++ }
                    1 -> npcl(FaceAnim.FRIENDLY, "Don't suppose you've seen any boots around here? Size nine, steel toed, brown leather...").also { stage++ }
                    2 -> npcl(FaceAnim.FRIENDLY, "Nope, not at all. I'd best be going. Have a nice day. Bye!").also { stage = END_DIALOGUE }
                }
            }
        }
        return true
