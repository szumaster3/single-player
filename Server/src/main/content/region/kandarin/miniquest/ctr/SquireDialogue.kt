...
class SquireDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when {
            getAttribute(player, GameAttributes.KW_BEGIN, false) -> npc("Good day, my lord. Is there anything I can do", "for you?").also { stage = 14 }
            getAttribute(player, GameAttributes.KW_COMPLETE, false) -> npc("Congratulations on succeeding in the Knight Waves,", "${if (!player.isMale) "my lady" else "my lord"}.").also { stage = 29 }
