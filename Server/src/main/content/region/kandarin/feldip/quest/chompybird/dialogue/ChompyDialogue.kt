...
class ChompyDialogue(val quest: Quest) : DialogueFile() {
    ...
    private fun handleImpatienceDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            4 -> when (buttonId) {
                1 -> playerl("How do I make the 'stabbers' again?").also { stage++ }
            ...
            101 -> {
                npcl(FaceAnim.OLD_NORMAL, "Ahh, der creature has dem...goodly, goodly. Now us can stick der chompy bird...")
    ...
    private fun handleToadsiesDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            5 -> when (buttonId) {
                1 -> playerl("How do we make the chompys come?").also { stage++ }
    ...
    private fun handlePlaceToadsiesDialogue(player: Player?, buttonId: Int) {
        npcl(FaceAnim.OLD_NORMAL, "Over 'dere creature, put da toadies over der! ~ The ogre points to a clearing to the south. ~")
    ...
    private fun handlePlacedBaitDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            ...
            1 -> npcl(FaceAnim.OLD_NORMAL, "Goodz me now waits for da chompy! It shouldn't be long now. Sneaky...sneaky...").also { stage++ }
            2 -> playerl("Yes, I know...stick da chompy!").also { stage++ }
            3 -> npcl(FaceAnim.OLD_NORMAL, "Hey.. you's creature, is da fatsy toady still dere? Go get more fatsy toadies if dey all gone!").also { stage++ }
    ...
    private fun handleRantzSucksDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            ...
            1 -> npcl(FaceAnim.OLD_NORMAL, "I knows, I keeps missing... because your stabbers are worserer at flying than a dead dog.").also { stage++ }
            ...
            3 -> when (buttonId) {
                ...
                2 -> playerl("Come on, let me have a go...").also { stage++ }
            }
            ...
            6 -> when (buttonId) {
                1 -> playerl("I'm actually quite strong... please let me try.").also { stage++ }
                2 -> playerl("Oh suit yourself, you'll just have to go hungry.").also { stage = END_DIALOGUE }
            }
            7 -> npcl(FaceAnim.OLD_NORMAL, "Oh, ok... I lend you other stabby thrower... but creature don't better cry when it hurts itself.").also { stage++ }
            8 -> {
    ...
    private fun handleWaitingForChompyDialogue(player: Player?, buttonId: Int, ) {
        ...
        when (stage) {
            ...
            101 -> npcl(FaceAnim.OLD_NORMAL, "Dat's a great chompy, you musta got a lucky shot wiv da stabbie chucker.").also { stage++ }
            102 -> npcl(FaceAnim.OLD_NORMAL, "Okay's now you's needs to cook da chompy! Slurp! You's can cook it's over der! ~Rantz points to a nearby spit roast.~").also { stage++ }
            103 -> npcl(FaceAnim.OLD_NORMAL, "But's we's particular about our chompy yumms. Me's wants ${getItemName(rantzIngredient)} wiv mine! Fycie and Bugs want something wiv der's as well, go and ask 'em wat dey want.",).also { stage++ }
            104 -> playerl("What! Now I've got the chompy bird, you expect me to cook it as well?").also { stage++ }
            105 -> npcl(FaceAnim.OLD_NORMAL, "Yep, da spit's over der! Last time Rantz did yummies, got very bad, did bad things to food.. and belly.").also { stage++ }
            106 -> {
                playerl("HUH!")
    ...
    private fun handleWaitingForCookedChompy(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            101 -> npcl(FaceAnim.OLD_NORMAL, "Hey hey! We got da delicious chompy bird Yay! This looks really tasty as well!").also { stage++ }
            102 -> npcl(FaceAnim.OLD_NORMAL, "Tank's very much for da chompy... Fycie an Bugs like very much da chompy yumms!").also { stage++ }
            ...
            105 -> playerl("It's my pleasure!").also {
                quest.finish(player)
                stage = END_DIALOGUE
            }
        }
