...
class BugsChompyDialogueFile(val quest: Quest) : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (quest.getStage(player)) {
            in 0 until 20 -> npcl(FaceAnim.OLD_NORMAL, "You's better talk to Dad, him chasey sneaky da chompy.").also { stage = END_DIALOGUE }
    ...
    private fun handleIngredientDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            0 -> npcl(FaceAnim.OLD_NORMAL, "Dad say's you's making da chompy for us! Slurp! Me's has to have ${getItemName(bugsIngredient)} wiv mine! Chompy is our favourite yummms!").also {
