        ...
        when (stage) {
            ...
            1 -> {
                if (hasTalkWithSithik) {
                    ...
                    openDialogue(p, ZavisticRarveLastDialogue())
                } else {
            ...
            3 -> {
                ...
                val nextDialogue = when {
                    hasBlackPrismAndTornPage -> ZavisticRarveHasBothItemsDialogue()
                    hasBlackPrism -> ZavisticRarveBlackPrismDialogue()
                    hasTornPage -> ZavisticRarveTornPageDialogue()
                    hasTankard -> ZavisticRarveTankardDialogue()
                    questComplete -> ZavisticRarveDefaultDialogue()
                    hasOrLostStrangePotion -> ZavisticRarvePotionDialogue()
                    else -> ZavisticRarveDefaultQuestDialogue()
                }
                openDialogue(p, nextDialogue)
            }
        }
    }
}
    ...
    override fun handle(
        ...
        when (stage) {
            ...
            1 -> when (buttonID) {
                ...
                2 -> playerl(FaceAnim.HALF_GUILTY, "What are the requirements to get in the Wizards' Guild?").also {
                    stage = 4
                }
