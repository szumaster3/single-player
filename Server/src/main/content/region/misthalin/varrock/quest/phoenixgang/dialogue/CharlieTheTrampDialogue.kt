...
class CharlieTheTrampDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            50 -> npcl(FaceAnim.FRIENDLY, "Charles. Charles E. Trampin' at your service. Now, about that change you were going to give me...").also { stage = 0 }
            ...
            203 -> when (buttonId) {
                ...
                2 -> player(FaceAnim.HALF_THINKING, "So... don't I get some sort of quest hint or something now?").also { stage = 220 }
            }
            220 -> npcl(FaceAnim.ANNOYED, "Huh? What do you mean? That wasn't why I asked you for money.").also { stage++ }
            221 -> npcl(FaceAnim.SAD, "I just need to eat...").also { stage = END_DIALOGUE }
            250 -> npcl(FaceAnim.AFRAID, "The ruthless and notorious criminal gang known as the Black Arm Gang have their headquarters down there.").also { stage++ }
            ...
            280 -> if (!ShieldofArrav.isBlackArm(player) && !ShieldofArrav.isPhoenix(player)) {
                npcl(FaceAnim.SUSPICIOUS, "You never know. You'll find a lady down there called Katrine. Speak to her.").also { stage = 282 }
                ...
                npcl(FaceAnim.ANNOYED, "No. You're a collaborator with the Phoenix Gang. There's no way they'll let you join now.").also { stage++ }
            }

            282 -> npc(FaceAnim.AFRAID, "But don't upset her, she's pretty dangerous.").also { stage++ }
            283 -> npcl(FaceAnim.FRIENDLY, "I also heard that Reldo the librarian knows more about them, go talk to him.").also { stage++ }
            ...
            292 -> when (buttonId) {
                1 -> player(FaceAnim.SUSPICIOUS, "How did you know I was in the Phoenix Gang?").also { stage = 300 }
                2 -> player(FaceAnim.SUSPICIOUS, "Any ideas how I could get in there then?").also { stage = 302 }
            }
            300 -> npcl(FaceAnim.NEUTRAL, "In my current profession I spend a lot of time on the streets and you hear these sorta things sometimes.").also { stage++ }
            ...
            303 -> npcl(FaceAnim.THINKING, "Your best bet would probably be to find someone else... Someone who ISN'T a member of the Phoenix Gang, and get them to infiltrate the ranks of the Black Arm Gang for you.").also { stage++ }
            304 -> npc(FaceAnim.THINKING, "If you find someone like that, tell 'em to come to me first.").also { stage++ }
            ...
            320 -> npcl(FaceAnim.NEUTRAL, "There's plenty of other adventurers about besides yourself. I'm sure if you asked one of them nicely they would help you.").also { stage = END_DIALOGUE }
        }
        return true
    }
