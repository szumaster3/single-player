...
/**
 * Represents the dialogue file for King Roald.
 *
 * # Relations
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        if (player!!.inventory.containsItem(ShieldofArrav.PHOENIX_SHIELD) || player!!.inventory.containsItem(ShieldofArrav.BLACKARM_SHIELD)) {
            ...
            when (stage) {
                ...
                1 -> interpreter!!.sendItemMessage(if (player!!.inventory.containsItem(ShieldofArrav.BLACKARM_CERTIFICATE)) {
                        ShieldofArrav.BLACKARM_CERTIFICATE.id
                    } else {
                        ShieldofArrav.PHOENIX_CERTIFICATE.id
                    }, "You show the certificate to the king.",).also {
                        stage++
                    }
            ...
            when (stage) {
                ...
                5 -> {
                    ...
                    if (player!!.inventory.remove(CERTIFICATE)) {
                        if (!player!!.inventory.add(Item(Items.COINS_995, 600))) {
                            GroundItemManager.create(Item(Items.COINS_995, 600), player!!)
                        }
