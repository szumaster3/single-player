...
class LumbridgeCookDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (getQuestStage(player, Quests.COOKS_ASSISTANT) <= 0) {
            ...
            if (getAttribute(player, GameAttributes.ALL_SUBMITTED, false) || (getAttribute(player, GameAttributes.MILK_SUBMITTED, false) && getAttribute(player, GameAttributes.FLOUR_SUBMITTED, false) && getAttribute(player, GameAttributes.EGG_SUBMITTED, false))) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            50 -> npc(FaceAnim.HAPPY, "Oh thank you, thank you. I need milk, an egg and", "flour. I'd be very grateful if you can get them for me.").also {
            ...
            70 -> npc(
                ...
                "though, " + if (inInventory(player, EMPTY_POT, 1)) {
            ...
            100 -> if (!getAttribute(player, GameAttributes.MILK_SUBMITTED, false) && inInventory(player, MILK, 1)) {
                setAttribute(player, "/save:${GameAttributes.MILK_SUBMITTED}", true).also {
                    player(FaceAnim.HAPPY, "Here's a bucket of milk.")
                    player.inventory.remove(Item(MILK))
                    gave = true
                    stage = 100
                }
            } else if (!getAttribute(player, GameAttributes.FLOUR_SUBMITTED, false) && inInventory(player, FLOUR, 1)) {
                setAttribute(player, "/save:${GameAttributes.FLOUR_SUBMITTED}", true).also {
                    player(FaceAnim.HAPPY, "Here's a pot of flour.")
                    player.inventory.remove(Item(FLOUR))
                    gave = true
                    stage = 100
                }
            } else if (!getAttribute(player, GameAttributes.EGG_SUBMITTED, false) && inInventory(player, EGG, 1)) {
                setAttribute(player, "/save:${GameAttributes.EGG_SUBMITTED}", true).also {
                ...
                if (gave) {
                    if (!player.getAttribute(GameAttributes.PART_SUBMITTED, false)) {
                        setAttribute(player, "/save:${GameAttributes.PART_SUBMITTED}", true)
                    }
                    if (getAttribute(player, GameAttributes.MILK_SUBMITTED, false) && getAttribute(player, GameAttributes.FLOUR_SUBMITTED, false) && getAttribute(player, GameAttributes.MILK_SUBMITTED, false)) {
                        npc(FaceAnim.HAPPY, "You've brought me everything I need! I am saved!", "Thank you!").also {
                            setAttribute(player, "/save:${GameAttributes.ALL_SUBMITTED}", true)
                            stage = 200
                        }
                    } else {
                    ...
                    if (!getAttribute(player, GameAttributes.PART_SUBMITTED, false)) {
            ...
            151 -> leftoverItems = "".also {
                if (!getAttribute(player, GameAttributes.MILK_SUBMITTED, false)) {
                    leftoverItems += "A bucket of milk. "
                }
                if (!getAttribute(player, GameAttributes.FLOUR_SUBMITTED, false)) {
                    leftoverItems += "A pot of flour. "
                }
                if (!getAttribute(player, GameAttributes.EGG_SUBMITTED, false)) {
            ...
            161 -> when (buttonId) {
                ...
                2 -> player(FaceAnim.ASKING, "Can you remind me how to find these things again?").also { stage = 60 }
            }
            ...
            300 -> if (getQuestStage(player, Quests.THE_LOST_TRIBE) == 10) {
