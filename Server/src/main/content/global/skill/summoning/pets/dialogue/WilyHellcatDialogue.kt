...
class WilyHellcatDialogue : Dialogue {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> playerl(FaceAnim.HALF_ASKING, "How are you doing?").also { stage++ }
            1 -> npcl(FaceAnim.CHILD_NORMAL, "I'm as happy as a demon in a lava pit!").also { stage++ }
            2 -> playerl(FaceAnim.FRIENDLY, "How old are you now?").also { stage++ }
            3 -> npcl(FaceAnim.CHILD_NORMAL, "I'm feeling a bit like a ghost in a cake shop.").also {  stage = END_DIALOGUE }
        }
        return true
    }
