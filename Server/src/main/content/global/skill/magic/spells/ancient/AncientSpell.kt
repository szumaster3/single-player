...
class AncientSpell private constructor(
    ...
    *definition.runes
    ...
    override fun fireEffect(entity: Entity, victim: Entity, state: BattleState) {
        when {
            definition.name.startsWith("BLOOD") -> {// Blood spells
            ...
            definition.name.startsWith("ICE") -> {// Ice spells
            ...
            definition.name.startsWith("SMOKE") -> {// Smoke spells
                if (state.estimatedHit > -1) {
    ...
    fun shouldUseMultiHit(entity: Entity, target: Entity, isSingleTarget: Boolean): Boolean {
    ...
    override fun getTargets(entity: Entity, target: Entity): Array<BattleState> {
        val isSingleTarget = when (definition.type) {
    ...
    override fun getMaximumImpact(entity: Entity, victim: Entity, state: BattleState): Int {
        val add = when {
            ...
            definition.name.startsWith("MIA") -> when (definition.type) {
                SpellType.RUSH -> 4
    ...
    override fun cast(entity: Entity, target: Node): Boolean {
        if (definition.name.startsWith("MIA")) {
            val player = entity as? Player
            val weapon = player?.equipment?.getNew(EquipmentContainer.SLOT_WEAPON)
            val valid = intArrayOf(Items.ZURIELS_STAFF_13867, Items.ZURIELS_STAFF_DEG_13869, Items.NULL_13841, Items.NULL_13843)
                .any { it == weapon?.id }
            if (!valid && core.game.world.GameWorld.settings?.isDevMode != true) {
                player?.packetDispatch?.sendMessage("You need to be wielding Zuriel's staff in order to cast this spell.")
                return false
            }
        }
