...
class MonkNPC(id: Int = 0, location: Location? = null) : AbstractNPC(id, location) {
    ...
    override fun construct(id: Int, location: Location, vararg objects: Any): AbstractNPC = MonkNPC(id, location)
    ...
    override fun finalizeDeath(killer: Entity?) {
        ...
        val hasKey = hasAnItem(player, Items.KEY_2834)?.container != null
        val hasClue = player.inventory.containsItem(Item(Items.CLUE_SCROLL_10222, 1))
        val inArea = inBorders(player, 2590, 3203, 2622, 3221)

        if (inArea && !hasKey && hasClue) {
            produceGroundItem(player, Items.KEY_2834, 1, location)
        }
