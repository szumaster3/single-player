...
class BeaconKeeperDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (index == AFUBeacon.MONASTERY.ordinal && getStatLevel(player, Skills.PRAYER) < 53) {
            npc(faceExpression, "I will aid you when your devotion is", "strong enough.", "(You need 53 Prayer for him to assist you.)").also { stage = 15 }
            return true
        }
            ...
            13 -> when (buttonId) {
                1 -> {
                    if (logs.id != 0) {
                        ...
                        npc(faceExpression, "How you expect me to tend fire for you? You not have enough good logs for me to use!").also { stage = END_DIALOGUE }
                    }
                }
                2 -> end()
            }
            14 -> npcl(faceExpression, "Thanks. I use these five logs to keep beacon burning whilst you away.").also { stage = END_DIALOGUE }
